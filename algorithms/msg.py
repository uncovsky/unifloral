...
class Args:
    ...
    num_critics: int = 10
    # --- MSG --- 
    msg_min_q_weight : float = 1.0
    msg_lcb_coefficient : float = 4.0
    ...
    def _train_step(runner_state, _):
        ...
        def _actor_loss_function(params, rng):
            def _compute_loss(rng, transition):
                ...
                q_std = q_values.std()
                q_lcb = q_values.mean() - args.msg_lcb_coefficient * q_std
                return -q_lcb + alpha * log_pi, -log_pi, q_lcb, q_std
        ...
        (actor_loss, (entropy, q_lcb, q_std)), actor_grad = _actor_loss_function(
        ...
        ood_actions = jax.vmap(_sample_actions, in_axes=(0, 0))(ood_rngs, batch.obs)

        # --- Update critics ---
        @jax.value_and_grad
        def _q_loss_fn(params):
            q_pred = q_apply_fn(params, batch.obs, batch.action)
            q_ood = q_apply_fn(params, batch.obs, ood_actions)
            critic_loss = jnp.square(q_pred - target).sum(-1).mean()
            # OOD regularizer (Consistent with MSG implementation)
            ood_gap = q_ood.mean() - q_pred.mean()
            return critic_loss + args.msg_min_q_weight * ood_gap
