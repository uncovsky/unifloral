...
class Args:
    ...
    anchor_lagrangian: float = 1.0 # CQL penalty for anchor
...
AgentTrainState = namedtuple("AgentTrainState", "actor vec_q vec_q_target anchor_q anchor_q_target alpha pretrain_lag train_step")
...
def make_train_step(args, actor_apply_fn, q_apply_fn, anchor_apply_fn, alpha_apply_fn, dataset,
    ...
    def _train_step(runner_state, _):
        ...
        next_q_anchor = anchor_apply_fn(agent_state.anchor_q_target.params,
                            batch.next_obs, 
                            bootstrap_actions)
        next_q_anchor = next_q_anchor.min(-1, keepdims=True)
        ...
        td_target_anchor = jnp.expand_dims(batch.reward, -1) + args.gamma * jnp.expand_dims((1 - batch.done), -1) * next_q_anchor
        ...
        def _sample_actions(rng, obs, count):
            ...
            actions = pi.sample(seed=rng, sample_shape=(count,))
            return jnp.swapaxes(actions, 0, 1)

        pbrl_action_count = 10
        
        rng_curr, rng_next, rng_rand, rng = jax.random.split(rng, 4)
        # [B, count, action_dim]
        curr_actions = _sample_actions(rng_curr, batch.obs, pbrl_action_count)
        next_actions = _sample_actions(rng_next, batch.next_obs, pbrl_action_count)
        rand_actions = jax.random.uniform(
                rng_rand,
                (args.batch_size, pbrl_action_count, batch.action.shape[-1]),
                minval=-1.0,
                maxval=1.0,
        )

        states = jnp.expand_dims(batch.obs, axis=1).repeat(pbrl_action_count, axis=1)
        next_states = jnp.expand_dims(batch.next_obs, axis=1).repeat(pbrl_action_count, axis=1)

        @partial(jax.value_and_grad, has_aux=True)
        def _anchor_loss_fn(params):
            q_pred = anchor_apply_fn(params, batch.obs, batch.action)

            # td error
            td_error = jnp.square(q_pred - td_target_anchor).sum(-1).mean()
            q_pred = jnp.expand_dims(q_pred, axis=1)
            q_pi = anchor_apply_fn(params, states, curr_actions)
            q_pi_next = anchor_apply_fn(params, next_states, next_actions)
            q_rand = anchor_apply_fn(params, states, rand_actions)
            all_qs = jnp.concatenate([q_pred, q_pi, q_pi_next, q_rand], axis=1)
            q_ood = jax.scipy.special.logsumexp(all_qs / args.critic_regularizer_parameter, axis=1).sum(-1)

            loss = (q_ood.mean() - q_pred.mean()) * args.anchor_lagrangian + td_error
            return loss, (q_pi.min(-1, keepdims=True), q_pi_next.min(-1, keepdims=True))

        (anchor_loss, (anchor_q, anchor_q_next)), anchor_grad = _anchor_loss_fn(agent_state.anchor_q.params)

        updated_anchor = agent_state.anchor_q.apply_gradients(grads=anchor_grad)
        agent_state = agent_state._replace(anchor_q=updated_anchor)

        updated_target_params = optax.incremental_update(
                agent_state.anchor_q.params,
                agent_state.anchor_q_target.params,
                args.polyak_step_size,
        )

        updated_anchor_target = agent_state.anchor_q_target.replace(
                params=updated_target_params,
        )
        agent_state = agent_state._replace(anchor_q_target=updated_anchor_target)
        ...
        def _q_loss_fn(params):
            ...
            """
                Anchored PBRL loss
            """
            q_ood_curr = q_apply_fn(
                    params, states, curr_actions
            )

            std_q_ood_curr = q_ood_curr.std(-1, keepdims=True)
            ood_q_target = q_ood_curr - args.critic_lagrangian * std_q_ood_curr

            if args.anchor_enabled:
                ood_q_target = jnp.maximum(ood_q_target, anchor_q)

            ood_q_target = jax.lax.stop_gradient(ood_q_target)

            critic_regularizer_loss = jnp.square(q_ood_curr - ood_q_target).sum(-1).mean()

            critic_loss += regularizer_loss * args.reg_lagrangian
...
def train(args):
    ...
    # CQL anchor
    anchor_net = VectorQ(2, critic_norm=args.critic_norm)
    ...
    rng_q, rng_anchor = jax.random.split(rng_q, 2)
    agent_state = AgentTrainState(
        ...
        anchor_q=create_train_state(args, rng_anchor, anchor_net, [dummy_obs,  dummy_action]),
        anchor_q_target=create_train_state(args, rng_anchor, anchor_net, [dummy_obs,  dummy_action]),
    ...
    _agent_train_step_fn = make_train_step(
        args, actor_net.apply, q_apply, anchor_net.apply, alpha_net.apply, dataset, 
    ...
    for eval_idx in tqdm.tqdm(range(num_evals), desc="train epochs"):
        ...
        # --- Visualize Q-values ----
        def visualize_q_vals():
            states = dataset.obs[0].reshape(1, -1).repeat(1000, axis=0)
            actions = jnp.linspace(-args.action_scale, args.action_scale, 1000).reshape(-1, 1)
            anchor_values = anchor_net.apply(agent_state.anchor_q.params, states, actions)
            q_values = q_apply(agent_state.vec_q.params, states, actions)
            plt.plot(actions, q_values)
            plt.plot(actions, anchor_values, linestyle="dashed")
            plt.show()

        if args.dataset_name == "bandit_expert-v0":
            visualize_q_vals()
