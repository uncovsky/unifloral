...
from infra.utils import linear_schedule
...
def make_train_step(args, actor_apply_fn, q_apply_fn, alpha_apply_fn, dataset):
    ...
    schedule_fn = linear_schedule(start=0.0, end=1.0,
                                  max_steps=50000)

    def _train_step(runner_state, _):
        rng, agent_state = runner_state

        trust_reg = schedule_fn(agent_state.train_step)
        ...
        def _q_loss_fn(params):
            ...
            stds = all_qs.std(axis=-1).mean(1)
            normalized_stds = stds / ( jnp.mean(jnp.abs(all_qs)) + 1e-7)
            lmb = 10.0 / args.num_critics
            unc_weights = jnp.exp(-lmb * normalized_stds)
            unc_weights = unc_weights.clip(1.0 - trust_reg, 1.0)
            unc_weights = jax.lax.stop_gradient(unc_weights)

            q_ood = jax.scipy.special.logsumexp(all_qs / args.cql_temperature, axis=1).sum(-1)
            q_ood = q_ood * args.cql_temperature
            q_gap = q_ood - q_pred.mean(-1)
            min_q_loss = args.cql_min_q_weight * (unc_weights * q_gap).mean()
            critic_loss += min_q_loss
            return critic_loss, (unc_weights.min(), unc_weights.mean())

        (critic_loss, (uw_min, uw_mean)), critic_grad = _q_loss_fn(agent_state.vec_q.params)
        ...
        agent_state = agent_state._replace(
            train_step=agent_state.train_step + 1
        )
